...
export class FileFromUploaded implements UploadHandlerInterface<UploadcareFileInterface, FileUploadLifecycleInterface> {
  ...
  upload(fileUploadLifecycle: FileUploadLifecycleInterface): Promise<UploadcareFileInterface> {
    ...
    return this.request
      .then(({uuid}) => fileUploadLifecycle.handleUploadedFile(uuid, this.settings))
