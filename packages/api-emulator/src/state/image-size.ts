...
/**
 * The dimensions the fake API reports for an upload, read out of the bytes
 * themselves rather than trusted from the declared mime type — the suite
 * uploads a PNG named `square.jpg` as `image/jpeg`, and the real API answers
 * with what it decoded, not with what it was told. Bytes that decode as nothing
 * are a non-image: `is_image: false`, no `image_info`, which is the same answer
 * the real API gives the fixture whose contents are the word "content".
 *
...
export const imageSize = (bytes: Uint8Array): StoredImage | undefined => {
  ...
  if (bytes.byteLength > 4 && at(0) === 0xff && at(1) === 0xd8) {
    ...
    while (offset + 9 < bytes.byteLength) {
      ...
      offset +=
        marker >= 0xd0 && marker <= 0xd9 ? 2 : 2 + view.getUint16(offset + 2)
    }
  }
