...
`CdnUrl` can hold a `conversion: 'video'` and will happily serialize it, but it attaches the CDN base and hands back a full URL. The convert API wants a path, so use `videoPath` or a fluent video chain instead. Conversion paths are the one thing here that needs no [CDN base](/guide/cdn-base) at all; `gif2video`, being an on-the-fly URL, does. The [string level](/guide/string-level-api) has no place in either: it builds URLs, not paths, and a `gif2video` prefix lives inside what it treats as the chain, so replacing that chain drops the conversion.
:::
...
- `thumbs(n)` accepts 1 to 50 and must be the last operation
...
Animated GIFs convert to video directly on the CDN, with no REST job:
...
Note the URL shape: the `gif2video` prefix attaches right after the uuid with a plain `/`, without a `-/` separator. The source file must be an animated image, otherwise the CDN responds `400`.
