...
Every Uploadcare CDN URL follows one structure. Once you know it, the parser's output (and the rest of this library) is obvious.
...
The shape only has the fields its kind allows, and TypeScript narrows on `kind`:
...
| `prefixed` | `<prefix>.ucarecd.net` | per-project subdomain, computed from your public key. Yes, `ucarecd.net` is a different zone than `ucarecdn.com`, not a typo |
...
`?token=…` (secure delivery signatures) and `#fragments` are preserved verbatim through parse → serialize. The library never generates tokens. Remember that editing operations invalidates an existing signature, since it was computed over the old path.
