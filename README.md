...
* [**@uploadcare/signed-uploads**](./packages/signed-uploads/README.md) — Credentials for Uploadcare's [Signed Uploads][uc-docs-signed-uploads] security feature. On the server it mints JWTs for the `Authorization: Bearer` scheme, with optional endpoint scope and operation limits, and the [legacy `{secureSignature, secureExpire}` pair][uc-docs-signature]. In the browser, `@uploadcare/signed-uploads/client` caches a token and replaces it before it expires. Supports flexible expiration options (absolute timestamps, `Date` objects, or relative lifetimes). Zero dependencies, full TypeScript support.
