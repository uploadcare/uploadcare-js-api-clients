...
* [**@uploadcare/cdn-url**](./packages/cdn-url/README.md) — Build and parse Uploadcare CDN URLs: typed image transformation operations with eager validation, file groups (`uuid~N`, `nth`, archives), delivery proxy URLs, and video/gif2video/document conversion paths. Supports legacy `ucarecdn.com`, prefixed `*.ucarecd.net`, custom CNAME and proxy domains. Lenient round-trip parser, cross-operation chain validation, atomic tree-shakeable entry points.
